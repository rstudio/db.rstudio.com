...
- **Actively developed, best-in-class packages**. These packages include: `dplyr`, `odbc`, `DBI`, `keyring`, `pool` and others.

- **Integration with RStudio.**  Starting with version 1.1, it is now possible to perform tasks like navigate database objects inside the RStudio interface.

- **Education is key.**  The goal of this website is to gather Best Practice, How-to and other helpful articles in one location.  RStudio is also working on other channels, like upcoming webinars and in-person training at **[rstudio::conf 2018](https://www.rstudio.com/conference/)** 
...
The RStudio **Connections Pane** makes it possible to easily connect to a variety of data sources, and explore the objects and data inside the connection. It extends, and is designed to work with, a variety of other tools for working with databases in R. Also, preview the first 1,000 records in a database by simply clicking on the table icon.

The Connections Pane is currently available only in the [preview release of RStudio 1.1](https://www.rstudio.com/products/rstudio/download/preview/). Make sure to use the latest version of the [odbc](/odbc) package.

Read more about this feature here: [Using RStudio Connections](https://support.rstudio.com/hc/en-us/articles/115010915687)
...
<div>
 ...
 4. **All your code is in R** - Because`dplyr` is used to communicate with the database, there is no need to alternate between languages, or tools, to perform the data exploration.
