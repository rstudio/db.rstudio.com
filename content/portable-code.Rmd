...
When we share R scripts, RNotebooks or other kind of content with others, and want the code to also execute in their enrionment, then we need to take steps to make the code portable. There are two primary considerations when sharing content that contain database
connections:
...
One way to manage ODBC connections is through DSNs. DSNs define the details for
...
The key is to ensure the DSN in the development environment and the DSN in the
...
fail. In other cases, it is possible the DSNs will be similar enough for
...
plain text storage. It is common that the method used to access credentials
locally may vary from the method used for deployed content. For example, the
