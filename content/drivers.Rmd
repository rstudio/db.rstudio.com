...
[unixODBC](http://www.unixodbc.org/). Drivers compiled against [iODBC](http://www.iodbc.org/) _may_ also work, but are not fully supported.
...
installer that needs to be run to install the drivers in the proper
locations.
...
### Installation
 ...
 1. Install [homebrew](http://brew.sh/) to easily install database drivers on MacOS
...
The [apt-get](https://wiki.debian.org/Apt) command can be used to easily install database
drivers on Linux distributions that support it, such as Debian and Ubuntu.
...
For database specific settings see go to the **Databases** section in the menu and look for the page that matches the desired database type.

### DSN Configuration files

ODBC configuration files are another option to specify connection parameters; they
allow one to use a Data Source Name (DSN) to make it easier to connect to a database.
...
- [Securing Credentials](/managing-credentials)

- [Making Scripts Portable](/portable-code)

- [Securing Deployed Content](/deployment)
